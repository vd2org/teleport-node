...
docker run -d --name teleport-node \
  ...
  ghcr.io/vd2org/teleport-node:v0.0.0
```
...
```shell
curl https://raw.githubusercontent.com/vd2org/teleport-node/v0.0.0/compose.yml |
...
```shell
curl https://raw.githubusercontent.com/vd2org/teleport-node/v0.0.0/compose.yml | docker compose -p teleport-node -f - up -d
```
